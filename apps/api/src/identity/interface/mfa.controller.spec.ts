...
describe('MfaController', () => {
  it('reports MFA status', async () => {
    const controller = controllerWith({ status: () => Promise.resolve({ enabled: true }) });
    expect(await controller.status(token)).toEqual({ enabled: true });
  });
  ...
  it('maps a missing user on enrollment to a 401 problem', async () => {
    const controller = controllerWith({
  ...
  it('rejects a malformed activation code with a 422 problem', async () => {
    const controller = controllerWith({
  ...
  it('activates with a valid code', async () => {
    const controller = controllerWith({
      activate: () => Promise.resolve({ ok: true, value: undefined }),
    });
    expect(await controller.activate(token, { code: '123456' })).toEqual({ status: 'active' });
  });
